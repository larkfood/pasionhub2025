			...
			foreach ($this->customFields as $field) {
				...
				if ($field->type == 'file') {
					...
					if ($request->filled('post_id')) {
						...
						$postValue = PostValue::where('post_id', $request->input('post_id'))
					...
					$cfRules[] = 'mimes:' . $allowedFileFormats;
	...
	public function messages(): array
	{
		...
		if ($this->customFields->count() > 0) {
			foreach ($this->customFields as $field) {
				...
				if ($field->required == 1) {
					$messages['cf.' . $field->id . '.required'] = t('The field is required', [
						'field' => mb_strtolower($field->name),
					]);
				}
				
				if ($field->type == 'url' || $field->type == 'video') {
					$messages['cf.' . $field->id . '.url'] = t('The field field must be a valid URL', [
						'field' => mb_strtolower($field->name),
					]);
				}
				
				if ($field->type == 'number') {
					$messages['cf.' . $field->id . '.integer'] = t('The field field must be a number', [
						'field' => mb_strtolower($field->name),
					]);
				}
				...
				if ($field->type == 'file') {
					$messages['cf.' . $field->id . '.mimes'] = t('The file of field must be in the good format', [
						'field' => mb_strtolower($field->name),
					]);
